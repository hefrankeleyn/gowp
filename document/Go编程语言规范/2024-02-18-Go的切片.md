...
这个代码就像描述的一样，但是它返回的`[]byte`是指向一个包含了完整文件内容的数组。因为这个切片引用原始数组，只要这个切片保留在垃圾回收器周围，数组就不会被释放。文本中少量有用的字节使整个内容保留在内存中。

为了修复这个问题，一个方法是在返回它之前拷贝有用的数组到一个新的切片中。
...
[Effective Go](https://go.dev/doc/effective_go)包含了深入处理[切片](https://go.dev/doc/effective_go#slices)和[数组](https://go.dev/doc/effective_go#arrays)，并且在[Go的语言规范](https://go.dev/ref/spec)定义[切片](https://go.dev/ref/spec#Slice_types)和它们相关有用的函数。

- [effective_go#slices](https://go.dev/doc/effective_go#slices)
- [effective_go#arrays](https://go.dev/doc/effective_go#arrays)
- [spec#Slice_types](https://go.dev/ref/spec#Slice_types)
- [spec#Length_and_capacity](https://go.dev/ref/spec#Length_and_capacity)
- [spec#Making_slices_maps_and_channels](https://go.dev/ref/spec#Making_slices_maps_and_channels)
- [spec#Appending_and_copying_slices](https://go.dev/ref/spec#Appending_and_copying_slices)
